...
global.grav = 0.3;
global.jump_height = 2.5;
global.jump_invincibility = false;
global.wall_height = 2;
	...
	if(instance_exists(Player)){
	    with(Player){
    		if (!instance_exists(PlayerSit)){
    			...
    			if(button_check(index, jump_button) && z = 0 && jumping){
...
#define draw_all
    ...
    plane_setup();
        ...
        var _surf = global.surfFloor,
            ...
            _draw = false,
            f = instances_matching(Floor, "floor_surface_3D", null);
	...
	plane_setup();
    	...
    	with instances_viewbounds(40, _planeProj) {
	...
	with instances_viewbounds(40, [hitme, Revive]) {
	    ...
	    if(object_index == Player){
            ...
            for(var i = 0; i < array_length(_wep); i++){
	            ...
	            batch_add(_spr, 0, x + lengthdir_x(_ox, gunangle) + lengthdir_x(_oy, gunangle - 90), y + lengthdir_y(_ox, gunangle) + lengthdir_y(_oy, gunangle - 90), _oz, -(gunangle + w.wang), false, 1);
            }
		}
